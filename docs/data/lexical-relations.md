...
python tools\vocabulary\build_relations.py --vocabulary data\ielts\vocabulary.sqlite3 --oewn data\sources\oewn\english-wordnet-2025-json.zip --curated data\curated\confusable_groups.csv --misspellings data\curated\misspellings.csv --source-registry data\curated\source_registry.json --approval-policy data\curated\relation_approval_policy.json --output data\ielts\relations.sqlite3 --report data\reports\relations-quality.json --manifest data\ielts\relations-manifest.json
```

The builder loads the repository-owned OEWN hash, byte count, URL, core edition, and license from the strict source registry; callers cannot substitute a hash. A separately hard-pinned `relation_approval_policy.json` anchors the exact 18 group identities, classifications and ordered member sets, the exact two directional misspellings, and whole-file hashes for the reviewed Chinese contrast and collocation content. Any policy or reviewed-input drift is rejected before staging or promotion. The builder checks archive integrity before parsing, uses synset `partOfSpeech` as canonical POS, stages and independently reconstructs all relation sets and metrics, and promotes database/report/manifest with rollback and manifest last. See the shipped OEWN and Princeton WordNet license files in `data/licenses`.
