...
The builder accepts explicit source, curated, registry, and output paths. Ordinary candidate builds use `--output`; reproducible canonical promotion uses `--canonical-root`, which creates `data/ielts/{vocabulary.csv,vocabulary.sqlite3,manifest.json}` and `data/reports/vocabulary-quality.json` without manual copying or manifest editing. All paths are resolved before writing, inputs may not collide with output targets, and generation occurs in a temporary sibling tree. The staged artifacts pass the full verifier before the final files are replaced; replacement has rollback backups and promotes the manifest last.

The manifest records truthful paths relative to the final artifact directory plus hashes for the actual curated and registry inputs. Artifact paths must be relative and resolve inside explicitly allowed artifact/report roots. The verifier requires separately supplied curated and registry files, checks their manifest paths and hashes, derives required coverage from the curated file, and checks that the generated quality list agrees. It validates exact CSV and SQLite schemas, recomputes stable IDs independently in both artifacts, requires nonempty word/Chinese translation/provenance/selection reason fields in both artifacts, rejects undocumented provenance keys, compares every declared field in deterministic row order, and checks SQLite integrity, uniqueness, forbidden spellings, and all artifact hashes.
