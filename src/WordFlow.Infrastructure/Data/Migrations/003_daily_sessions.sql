CREATE TABLE daily_sessions (
    ...
    completed_at_utc TEXT NULL CHECK(completed_at_utc IS NULL OR (
        ...
        AND CAST(substr(completed_at_utc, 12, 2) AS INTEGER) BETWEEN 0 AND 23
        AND CAST(substr(completed_at_utc, 15, 2) AS INTEGER) BETWEEN 0 AND 59
        AND CAST(substr(completed_at_utc, 18, 2) AS INTEGER) BETWEEN 0 AND 59
    ...
    created_at_utc TEXT NOT NULL CHECK(
        ...
        AND CAST(substr(created_at_utc, 12, 2) AS INTEGER) BETWEEN 0 AND 23
        AND CAST(substr(created_at_utc, 15, 2) AS INTEGER) BETWEEN 0 AND 59
        AND CAST(substr(created_at_utc, 18, 2) AS INTEGER) BETWEEN 0 AND 59
    ...
    updated_at_utc TEXT NOT NULL CHECK(
        ...
        AND CAST(substr(updated_at_utc, 12, 2) AS INTEGER) BETWEEN 0 AND 23
        AND CAST(substr(updated_at_utc, 15, 2) AS INTEGER) BETWEEN 0 AND 59
        AND CAST(substr(updated_at_utc, 18, 2) AS INTEGER) BETWEEN 0 AND 59
        AND substr(updated_at_utc, 21, 7) NOT GLOB '*[^0-9]*')
) STRICT;

CREATE TABLE daily_queue_items (
    item_id TEXT PRIMARY KEY
        CHECK(length(item_id) = 36 AND item_id <> '00000000-0000-0000-0000-000000000000' AND lower(item_id) = item_id AND length(replace(item_id, '-', '')) = 32 AND replace(item_id, '-', '') NOT GLOB '*[^0-9a-f]*' AND substr(item_id, 9, 1) = '-' AND substr(item_id, 14, 1) = '-' AND substr(item_id, 19, 1) = '-' AND substr(item_id, 24, 1) = '-'),
    local_day TEXT NOT NULL REFERENCES daily_sessions(local_day)
        CHECK(length(local_day) = 10 AND local_day GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' AND date(local_day) IS local_day),
    ordinal INTEGER NOT NULL CHECK(ordinal >= 0),
    card_id TEXT NOT NULL
        CHECK(length(card_id) = 36 AND card_id <> '00000000-0000-0000-0000-000000000000' AND lower(card_id) = card_id AND length(replace(card_id, '-', '')) = 32 AND replace(card_id, '-', '') NOT GLOB '*[^0-9a-f]*' AND substr(card_id, 9, 1) = '-' AND substr(card_id, 14, 1) = '-' AND substr(card_id, 19, 1) = '-' AND substr(card_id, 24, 1) = '-'),
    ...
    source_item_id TEXT NULL REFERENCES daily_queue_items(item_id)
        CHECK(source_item_id IS NULL OR (length(source_item_id) = 36 AND source_item_id <> '00000000-0000-0000-0000-000000000000' AND lower(source_item_id) = source_item_id AND length(replace(source_item_id, '-', '')) = 32 AND replace(source_item_id, '-', '') NOT GLOB '*[^0-9a-f]*' AND substr(source_item_id, 9, 1) = '-' AND substr(source_item_id, 14, 1) = '-' AND substr(source_item_id, 19, 1) = '-' AND substr(source_item_id, 24, 1) = '-')),
    ...
    completed_event_id TEXT NULL REFERENCES review_event(event_id)
        CHECK(completed_event_id IS NULL OR (length(completed_event_id) = 36 AND completed_event_id <> '00000000-0000-0000-0000-000000000000' AND lower(completed_event_id) = completed_event_id AND length(replace(completed_event_id, '-', '')) = 32 AND replace(completed_event_id, '-', '') NOT GLOB '*[^0-9a-f]*' AND substr(completed_event_id, 9, 1) = '-' AND substr(completed_event_id, 14, 1) = '-' AND substr(completed_event_id, 19, 1) = '-' AND substr(completed_event_id, 24, 1) = '-')),
    created_at_utc TEXT NOT NULL CHECK(
        length(created_at_utc) = 28 AND created_at_utc NOT GLOB '*[^0-9TZ:.-]*'
        AND substr(created_at_utc, 11, 1) = 'T' AND substr(created_at_utc, 14, 1) = ':' AND substr(created_at_utc, 17, 1) = ':'
        AND substr(created_at_utc, 20, 1) = '.' AND substr(created_at_utc, 28, 1) = 'Z'
